...
const OverlaySlice = createSlice({
  ...
  reducers: {
    ...
    unsetOverlay: (
      ...
      ...state,
      currentOverlayIds: state.currentOverlayIds.filter(
        (overlayId) => overlayId !== action.payload
      ),
    }),
...
const isVisibleSelector = (state: { videoEditor: { overlay: OverlayState } }) =>
  state.videoEditor.overlay.isVisible
const overlayIdSelector = (state: { videoEditor: { overlay: OverlayState } }) =>
  state.videoEditor.overlay.currentOverlayIds
const currentlyEditedElementIdSelector = (state: {
  videoEditor: { overlay: OverlayState }
}) => state.videoEditor.overlay.currentlyEditedElementId
const currentlyEditedElementParentIdSelector = (state: {
  videoEditor: { overlay: OverlayState }
