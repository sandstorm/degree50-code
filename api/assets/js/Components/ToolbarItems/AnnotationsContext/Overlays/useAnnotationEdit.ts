...
export const useAnnotationEdit = (
  ...
  const [transientAnnotation, setTransientAnnotation] = useState<
    ...
    if (transientAnnotation) {
      setTransientAnnotation({
        ...transientAnnotation,
          ...
          clamp(
            timeToSecond(time),
            timeToSecond(transientAnnotation.start),
            duration
          )
        ),
      })
  ...
  const minAllowedEndTime = secondToTime(
    Math.min(
      timeToSecond(transientAnnotation?.start ?? minAllowedStartTime) + 1,
      duration
    )
  )
