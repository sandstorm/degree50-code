...
const getFocusNextElement = (
  annotationIds: Array<string>,
  deletedAnnotationId: string
) => {
  if (annotationIds.length > 1) {
    const deletedAnnotationIndex = annotationIds.findIndex(
      (id) => id === deletedAnnotationId
    )
    const annotationIdsWithoutDeletedElement = remove(
      annotationIds,
    ...
    const nextAnnotationIdToFocus =
      annotationIdsWithoutDeletedElement[
        Math.min(
          deletedAnnotationIndex,
          annotationIdsWithoutDeletedElement.length - 1
        )
      ]
    ...
    return () => {
      document
        .querySelector<HTMLElement>(
          `[data-focus-id="${nextAnnotationIdToFocus}"]`
        )
        ?.focus()
    }
  } else {
