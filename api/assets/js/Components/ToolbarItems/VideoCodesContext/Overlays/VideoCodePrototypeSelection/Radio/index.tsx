...
const Radio = (props: Props) => {
  ...
  const ariaLabel = `
        ...
        ${
          props.prototype.userCreated ? 'Selbsterstellter' : 'Vordefinierter'
        } Video Code.
        ${
          props.parentPrototype
            ? `Unter-Code von ${props.parentPrototype.name}.`
            : ''
        }
    `
  ...
  return (
    <li
      className={`video-code-select__option ${
        props.parentPrototype ? 'child-prototype' : ''
      }`}
    >
      <input
        {...inputProps}
        ref={ref}
        id={props.prototype.id}
