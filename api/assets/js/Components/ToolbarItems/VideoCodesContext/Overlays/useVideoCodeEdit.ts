...
import {
  adjustEndTimeByStart,
  secondToTime,
  timeToSecond,
} from 'Components/VideoEditor/utils/time'
import { ChangeEvent, useState } from 'react'

export const useVideoCodeEdit = (
  duration: number,
  initialVideoCode?: VideoCode
) => {
  const [transientVideoCode, setTransientVideoCode] = useState<
    VideoCode | undefined
  >(initialVideoCode)
    ...
    if (transientVideoCode) {
      setTransientVideoCode({
        ...transientVideoCode,
          ...
          clamp(
            timeToSecond(time),
            timeToSecond(transientVideoCode.start),
            duration
          )
        ),
      })
  ...
  const minAllowedEndTime = secondToTime(
    Math.min(
      timeToSecond(transientVideoCode?.start ?? minAllowedStartTime) + 1,
      duration
    )
  )
