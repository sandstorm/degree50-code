...
const mapStateToProps = (state: AppState) => {
  ...
  const currentlyEditedElementParentId =
    selectors.videoEditor.overlay.currentlyEditedElementParentId(state)
  const videoCodesById = selectors.data.videoCodePrototypes.selectById(state)

  const videoCodePrototype = currentlyEditedElementId
    ? videoCodesById[currentlyEditedElementId]
    : undefined
...
const EditVideoCodePrototypeOverlay: FC<Props> = (props) => {
  const { transientVideoCodePrototype, handleColorChange, handleNameChange } =
    useVideoCodePrototypeEdit(
  ...
  const overlayTitle = props.videoCodePrototype
    ? 'Video-Code bearbeiten'
    : 'Video-Code erstellen'
    ...
    <Overlay closeCallback={close} title={overlayTitle}>
      <div className={'new-video-code'}>
        <input
          ...
          aria-label={`Ausgewählte Code-Farbe: ${
            getColorName(transientVideoCodePrototype.color).name
          }.`}
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(memo(EditVideoCodePrototypeOverlay))
