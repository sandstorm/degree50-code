...
export const useCuttingMediaItemHandling = ({
  ...
  const updateMediaItem = useCallback(
    (
      item: MediaItem<Cut>,
      { start, end, ...rest }: { start?: string; end?: string; rest?: any[] }
    ) => {
      ...
      const adjustedDuration =
        Math.round(duration) < 1 ? 1 : Math.round(duration)
      ...
      const newValues = {
        ...
        // It would probably be better to find a way to make item interaction etc. to be controlled components instead or work with some form of intermediate data structure which can be reset.
          ...
          adjustedEnd === item.end
            ? d2t((t2d(adjustedEnd) + 0.01).toFixed(3))
            : adjustedEnd,
      }
...
export const addVideoContextPlayListElement = (
  ...
  const videoNode = videoCtx.video(
    newVideoElement,
    videoContextPlayListElement.offset,
    4,
    {
  ...
  videoNode.stop(
    videoContextPlayListElement.start + videoContextPlayListElement.duration
  )
 ...
 */
export const transformCutListToMediaItemsForCutContext = (
  cutList: CutList
): Array<MediaItem<Cut>> =>
  cutList
    .reduce(
