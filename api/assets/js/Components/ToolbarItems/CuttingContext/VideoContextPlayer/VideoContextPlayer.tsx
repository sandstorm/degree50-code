...
import {
  initVideoContext,
  addVideoContextPlayListElement,
  transformCutListToVideoContextPlayList,
} from '../util'
...
import {
  ...
  getAspectRatioWidth,
} from 'Components/VideoEditor/utils/aspectRatio'
...
const VideoContextPlayer: FC<Props> = (props) => {
  ...
  const videoContextPlayList = useMemo(
    () => transformCutListToVideoContextPlayList(props.cutList),
    [props.cutList]
  )
  ...
  const { width: videoContextPlayerWidth, height: videoContextPlayerHeight } =
    useDebouncedResizeObserver($videoContextPlayerRef, 100)

  // controls
  const { volume, setVolume, isMuted, toggleIsMuted } = useVideoContextVolume(
    videoContext,
    1
  )
  ...
  const { currentTime, setCurrentTime } =
    useVideoContextCurrentTime(videoContext)
  ...
  useEffect(() => {
    ...
    if (
      ...
      const aspectRatioHeight = getAspectRatioHeight(
        videoWidth,
        videoHeight,
        maxWidth
      )
      ...
      if (canvasHeight > containerHeight) {
        ...
        const aspectRatioWidth = getAspectRatioWidth(
          videoWidth,
          videoHeight,
          maxHeight
        )
    ...
    $canvasWrapperRef,
    videoSrcAttributes,
    videoContextPlayerWidth,
    videoContextPlayerHeight,
  ])
  ...
  useLayoutEffect(() => {
    ...
    const nodesAndElements = videoContextPlayList.map((cut) =>
  ...
  return (
    <div
      className="video-context-player"
      onKeyDown={handleKeyDown}
      ref={$videoContextPlayerRef}
    >
      <VideoContextCanvas
