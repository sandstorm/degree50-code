...
export const filterSlice = createSlice({
  ...
  reducers: {
    ...
    setGlobalSolutionFilter: (
      state: FilterState,
      action: PayloadAction<GlobalSolutionFilter>
    ) => {
      switch (action.payload) {
        case GlobalSolutionFilter.ALL:
          return {
            ...state,
            previousSolutions: state.previousSolutionIds.reduce<
              FilterState['previousSolutions']
            >(
              (acc, id) => ({
                ...acc,
...
const selectComponentsById = (state: FilterStateSlice) =>
  state.videoEditor.filter.components
const selectComponentIds = (state: FilterStateSlice) =>
  state.videoEditor.filter.componentIds
const selectPreviousSolutionsById = (state: FilterStateSlice) =>
  state.videoEditor.filter.previousSolutions
const selectPreviousSolutionIds = (state: FilterStateSlice) =>
  state.videoEditor.filter.previousSolutionIds

const selectComponents = createSelector(
  [selectComponentsById, selectComponentIds],
  (byId, ids) =>
    ids
      .map((id) => byId[id])
      .sort((component) => {
...
const selectPreviousSolutions = createSelector(
  [selectPreviousSolutionsById, selectPreviousSolutionIds],
  (byId, ids) => ids.map((id) => byId[id])
)

const selectVisiblePreviousSolutions = createSelector(
  [selectPreviousSolutions],
  (solutions) => solutions.filter((s) => s.isVisible)
)

const selectVisiblePreviousSolutionIds = createSelector(
  [selectVisiblePreviousSolutions],
  (solutions) => solutions.filter((s) => s.isVisible).map((s) => s.id)
)

const selectGlobalSolutionFilter = createSelector(
  [selectPreviousSolutions],
  (previousSolutions) => {
