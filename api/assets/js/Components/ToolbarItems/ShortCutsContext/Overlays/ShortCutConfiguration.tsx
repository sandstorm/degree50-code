...
const shortCutIdToLabelMap: Record<ShortCutId, string> = {
  ...
  [ShortCutId.SET_CURRENT_TIME_AS_START_VALUE]:
    'Aktuelle Player-Zeit als Startzeit verwenden',
  [ShortCutId.SET_CURRENT_TIME_AS_END_VALUE]:
    'Aktuelle Player-Zeit als Endzeit verwenden',
...
const mapStateToProps = (state: AppState, ownProps: OwnProps) => ({
  shortCutConfiguration: selectShortCutConfigurationById(
    state,
    ownProps.shortCutId
  ),
...
const ShortCutConfiguration = (props: Props) => {
  ...
  return (
    <Accordion
      title={title}
      buttonTitleClose="Bearbeitung beenden"
      buttonTitleOpen="TastenKombination bearbeiten"
    >
      <div className="short-cut-configuration">
        <div className="short-cut-configuration__modifier-list">
          {allShortCutModifiers.map((modifierId) => {
            ...
            const enabled =
              props.shortCutConfiguration.modifiers[modifierId].enabled
            const handleChange = () => {
