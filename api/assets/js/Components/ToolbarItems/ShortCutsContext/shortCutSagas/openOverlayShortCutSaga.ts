...
export const openOverlayAction = createAction(
  'SAGA/SHORT_CUT/OPEN_OVERLAY',
  (overlayId: string) => ({
    payload: { overlayId },
  })
)
...
function* openOverlay(action: ReturnType<typeof openOverlayAction>) {
  ...
  if (
    (overlayId === AnnotationOverlayIds.create &&
      !components.includes(TabsTypesEnum.VIDEO_ANNOTATIONS)) ||
    (overlayId === VideoCodeOverlayIds.create &&
      !components.includes(TabsTypesEnum.VIDEO_CODES)) ||
    (overlayId === CutOverlayIds.create &&
      !components.includes(TabsTypesEnum.VIDEO_CUTTING))
  ) {
