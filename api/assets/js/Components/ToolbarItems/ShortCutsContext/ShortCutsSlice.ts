...
export type ShortCutModifierConfigurations = Record<
  ShortCutModifierId,
  ShortCutModifierConfiguration
>
...
const ShortCutsSlice = createSlice({
  ...
  reducers: {
    setShortCutsState: (
      ...
      action: PayloadAction<ShortCutsState>
    ): ShortCutsState => {
    ...
    toggleModifierForShortCut: (
      ...
      if (
        allShortCutModifiers.filter(
          (id) => newState[shortCutId].modifiers[id].enabled
        ).length > 0
      ) {
        return newState
      }
...
export const selectShortCutsState = (state: { shortCuts: ShortCutsState }) =>
  state.shortCuts
export const selectShortCutConfigurationById = (
  state: { shortCuts: ShortCutsState },
  shortCutId: ShortCutId
) => state.shortCuts[shortCutId]
export const selectHotKeyByShortCutId = (
  state: { shortCuts: ShortCutsState },
  shortCutId: ShortCutId
 ...
 */
function createHotKeyFromShortCut(
