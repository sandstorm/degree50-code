...
const TimeInput = (props: Props) => {
  ...
  const focusFirstInput = useCallback(
    (ev: MouseEvent<HTMLLabelElement | HTMLDivElement>) => {
      // WHY: type issue
  ...
  const maxSeconds =
    timeValues.hours < maxHours || timeValues.minutes < maxMinutes
      ? 59
      : maxTimeValues.seconds
  const maxMs =
    timeValues.hours < maxHours ||
    timeValues.minutes < maxMinutes ||
    timeValues.seconds < maxSeconds
      ? 999
      : maxTimeValues.ms
  ...
  const minSeconds =
    timeValues.hours > minHours || timeValues.minutes > minMinutes
      ? 0
      : minTimeValues.seconds
  const minMs =
    timeValues.hours > minHours ||
    timeValues.minutes > minMinutes ||
    timeValues.seconds > minSeconds
      ? 0
      : minTimeValues.ms
