...
const getDurationForRenderConfig = (
  durationInPercentage: number,
  videoDuration: number
) => {
  return Math.max(
    5,
    Math.round((videoDuration / 100) * (100 - durationInPercentage))
  )
}
...
export const useMediaLaneRendering = ({
  ...
  const {
    ...
    ref,
  } = useDebouncedResizeObserver($mediaTrackRef, 500)
  ...
  const setRenderConfigForZoom = (zoomInPercent: number) => {
    const newRenderedDuration = getDurationForRenderConfig(
      zoomInPercent,
      videoDuration
    )
    ...
    const newTimelineStartTime = calculateTimelineStartTime(
      currentTime,
      newRenderedDuration,
      videoDuration
    )
  ...
  useEffect(() => {
    const newTimelineStartTime =
      currentTime > 0
        ? Math.floor(currentTime / renderConfig.duration) *
