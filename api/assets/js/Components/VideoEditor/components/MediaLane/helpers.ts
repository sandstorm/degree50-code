...
export const calculateTimelineStartTime = (
  currentTime: number,
  newRenderedDuration: number,
  ...
  const startTimeWithOutOfBoundsDifference =
    newRenderStartTime - outOfBoundsDifference
  const startTimeWithRightBoundAdjustment = Math.min(
    startTimeWithOutOfBoundsDifference,
    newRenderStartTime
  )
