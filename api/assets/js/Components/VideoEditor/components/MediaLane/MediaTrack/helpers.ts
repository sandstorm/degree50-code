...
const drawGrid = (
  ...
  const adjustmentFactor = getAdjustmentFactorForGrid(
    gridGap,
    gridNum,
    canvas.width
  )
...
const drawCursor = (
  ...
  ctx.fillRect(
    padding * gridGap + (currentTime - timelineStartTime) * gridGap * 10,
    0,
    pixelRatio,
    height
  )
...
const getAdjustmentFactorForGrid = (
  gridGap: number,
  gridNum: number,
  canvasWidth: number
) => {
