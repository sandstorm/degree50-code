...
const getVisibleItems = <T>(
  ...
  return items.filter((item) =>
    itemIsVisible(item, timelineStartTime, duration)
  )
}
...
export const useItemInteraction = <T>(
  ...
  const [lastTargetNode, setLastTargetNode] = useState<HTMLElement | undefined>(
    undefined
  )
  ...
  const onMouseUp = useCallback(() => {
    if (isDraging && lastTargetNode && lastDiffX && lastClickedItem) {
      ...
      if (lastClickedItemSide === 'left') {
        ...
        if (
          newStartTime >= 0 &&
          newStartTime < (lastClickedItem.endTime || 0)
        ) {
