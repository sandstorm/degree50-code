...
export const MediaItem = ({
  ...
  const positionLeft =
    ...
    (item.startTime - renderConfig.timelineStartTime) *
      renderConfig.gridGap *
      10

  const { showModal: showMediaItemModal, RenderModal: RenderMediaItemModal } =
    useModalHook()
      ...
      switch (item.originalData.type) {
        case MediaItemTypeEnum.annotation: {
          return (
            <AnnotationListItem
              ...
              annotationId={item.originalData.id}
              index={1}
            />
          )
        }
        ...
        case MediaItemTypeEnum.videoCode: {
          return (
            <VideoCodeListItem
              key={item.originalData.id}
              videoCodeId={item.originalData.id}
              index={1}
            />
          )
        }
