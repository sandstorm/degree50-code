...
export const itemIsVisible = <T>(
  ...
  const timelineClippingEnd =
    currentTimelineClippingStart + timelineClippingDuration

  return (
    item.startTime <= timelineClippingEnd &&
    item.endTime >= currentTimelineClippingStart
  )
}
...
export const getContextYPosition = ({
  ...
  return baseY >= mediaItemsHeight / 2
    ? baseY - 3 * contentMenuItemHeight
    : baseY
}
