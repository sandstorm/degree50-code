...
const mapStateToProps = (state: AppState) => {
  ...
  const previousSolutionsFromFilter =
    selectors.videoEditor.filter.selectPreviousSolutions(state)
  const previousSolutions = previousSolutionsFromFilter.map((s) => ({
    ...s,
    ...solutions[s.id],
  }))
  const globalSolutionFilter =
    selectors.videoEditor.filter.selectGlobalSolutionFilter(state)
...
const ActivePreviousSolutions = (props: Props) => {
  ...
  const globalSolutionFilterCheckboxAriaLabel = `${
    props.globalSolutionFilter === GlobalSolutionFilter.ALL
      ? 'Alle abwählen'
      : 'Alle auswählen'
  }`
        ...
        <>
          <div className="global-filter">
            <CheckboxWithIndeterminate
              id="globalFilter"
              value={mapGlobalSolutionFilterToCheckboxValue(
                props.globalSolutionFilter
              )}
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(React.memo(ActivePreviousSolutions))
