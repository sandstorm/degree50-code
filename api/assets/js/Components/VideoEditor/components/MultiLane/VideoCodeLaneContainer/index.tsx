...
const mergeCodesAndPrototypesToItems = (
  videoCodes: VideoCode[],
  prototypes: Record<string, VideoCodePrototype>
) => {
...
const VideoCodeLaneContainer = (props: Props) => {
  ...
  if (
    ...
    props.exercisePhaseType === ExercisePhaseTypesEnum.VIDEO_ANALYSIS
  ) {
    const mediaItems = mergeCodesAndPrototypesToItems(
      props.videoCodes,
      props.prototypes
    )
    const ownerName =
  ...
  return (
    <>
      {props.previousSolutions.map((solution) => {
        const videoCodes = solution.solutionLists.videoCodes.map(
          (id) => props.videoCodesById[id]
        )
        const mediaItems = mergeCodesAndPrototypesToItems(
          videoCodes,
          props.prototypes
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(React.memo(VideoCodeLaneContainer))
