...
import {
  playShortCutSuccessSoundAction,
...
export const setCurrentTimeAsStartValueAction = createAction(
  'SAGA/SHORT_CUTS/SET_CURRENT_TIME_AS_START_VALUE'
)
export const setCurrentTimeAsEndValueAction = createAction(
  'SAGA/SHORT_CUTS/SET_CURRENT_TIME_AS_END_VALUE'
)

export function* setCurrentTimeAsValueShortCutSaga() {
  yield takeLatest(
    setCurrentTimeAsStartValueAction,
    setCurrentTimeAsStartValueSaga
  )
...
function* setCurrentTimeAsStartValueSaga() {
  ...
  if (
    activeOverlayIds.some((overlayId) =>
      supportedOverlayIds.includes(overlayId)
    )
  ) {
...
function* setCurrentTimeAsEndValueSaga() {
  ...
  if (
    ...
    document
      .querySelector<HTMLButtonElement>(
        `button[data-short-cut-id="${ShortCutId.SET_CURRENT_TIME_AS_END_VALUE}"]`
      )
      ?.click()
