...
export const playShortCutSuccessSoundAction = createAction(
  'SAGA/SHORT_CUT/SHORT_CUT_SUCCESS'
)
export const playShortCutTriggerSoundAction = createAction(
  'SAGA/SHORT_CUT/SHORT_CUT_TRIGGER'
)
export const initializeSoundOptionsAction = createAction(
  'SAGA/SHORT_CUT/INITIALIZE_SOUND_OPTIONS'
)
export const persistSoundOptionsAction = createAction(
  'SAGA/SHORT_CUT/PERSIST_SOUND_OPTIONS'
)
...
function* initializeSoundOptions() {
  ...
  if (persistedShortCutSoundOptions) {
    const shortCutSoundOptionsFromStore = selectShortCutSoundState(
      yield selectState()
    )
    const shortCutSoundOptionsFromLocalStorage = JSON.parse(
      persistedShortCutSoundOptions
    ) as ShortCutSoundState

    const combinedState = merge(
      {},
      shortCutSoundOptionsFromStore,
      shortCutSoundOptionsFromLocalStorage
    )
...
function* persistSoundOptions() {
  ...
  localStorage.setItem(
    STORAGE_KEY,
    JSON.stringify(shortCutSoundOptionsFromStore)
  )
}
