...
export const getMinutesFromTimeSeconds = (seconds: number) =>
  Math.floor(seconds / 60) % 60
export const getHoursFromTimeSeconds = (seconds: number) =>
  Math.floor(seconds / 60 / 60)
 ...
 */
export const MinutesStringFormatter = Intl.NumberFormat(
  'de-DE',
  timeNumberFormat
)
 ...
 */
export const SecondsStringFormatter = Intl.NumberFormat(
  'de-DE',
  timeNumberFormat
)
 ...
 */
export const SecondsWithMillisecondsStringFormatter = Intl.NumberFormat(
  'de-DE',
  {
    ...timeNumberFormat,
...
export const sortByStartTime = <T extends { id: string; start: string }>(
  entities: T[]
): T[] =>
  [...entities].sort((a, b) => {
    if (a.start < b.start) {
...
export const adjustEndTimeByStart = (
  startTime: string,
  endTime: string,
  duration: number
) => {
  return secondToTime(
    clamp(
      ...
      timeToSecond(startTime) + DEFAULT_SECOND_STEP,
      duration
    )
  )
}
