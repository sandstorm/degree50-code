...
const mapStateToProps = (state: AppState) => ({
  visibleSolutions:
    selectors.videoEditor.filter.selectVisiblePreviousSolutions(state),
...
const PickComparedSolutionOverlay: FC<Props> = (props) => {
  ...
  const solutionOptions = visibleSolutions.map(({ id }) => (
    <RadioOption key={id} value={id}>
      {`${solutions[id].fromGroupPhase ? 'Gruppe von ' : ''}${
        solutions[id].userName ?? '<kein Name>'
      }`}
    </RadioOption>
  ))
  ...
  return (
    <Overlay
      title="Lösungen Auswählen und Vergleichen"
      closeCallback={handleCloseOverlay}
    >
      <Checkbox
        isSelected={props.shouldCompare}
        onChange={props.setShouldCompare}
      >
        Vergleichen
      </Checkbox>
