...
const mapStateToProps = (state: AppState) => ({
  solutionIds:
    selectors.videoEditor.filter.selectVisiblePreviousSolutionIds(state),
  selectedSolutionId:
    selectors.data.materialSolution.selectSelectedSolutionId(state),
  comparedSolutionId:
    selectors.data.materialSolution.selectComparedSolutionId(state),
  canSelectNextSolution:
    selectors.data.materialSolution.selectCanSelectNextSolution(state),
  canSelectPreviousSolution:
    selectors.data.materialSolution.selectCanSelectPreviousSolution(state),
  canCompareNextSolution:
    selectors.data.materialSolution.selectCanCompareNextSolution(state),
...
const MaterialSolution = (props: Props) => {
  ...
  const handleSelectNextSolution = () => {
    if (selectedSolutionId && canSelectNextSolution) {
      setSelectedSolutionId(
        solutionIds[solutionIds.indexOf(selectedSolutionId) + 1]
      )
    }
  }
  ...
  const handleCompareNextSolution = () => {
    if (comparedSolutionId && canCompareNextSolution) {
      setComparedSolutionId(
        solutionIds[solutionIds.indexOf(comparedSolutionId) + 1]
      )
    }
  }
