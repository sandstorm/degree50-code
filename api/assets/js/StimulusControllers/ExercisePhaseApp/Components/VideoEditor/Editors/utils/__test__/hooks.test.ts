...
describe('useMediaItemHandling()', () => {
    ...
    describe('updateMediaItems()', () => {
        describe('updateCallback and media item setter', () => {
            ...
            it('should be called when force is true (and not readOnly)', () => {
                const { result } = renderHook(() => useMediaItemHandling(baseConfig))

                act(() => result.current.updateMediaItems([], true, true))

                expect(baseConfig.setMediaItems).toHaveBeenCalled()
                expect(baseConfig.updateCallback).toHaveBeenCalled()
            })

            it('should not be called when force is true and readOnly', () => {
                const { result } = renderHook(() => useMediaItemHandling({ ...baseConfig, readOnly: true }))
            ...
            it('should be called when userIsCurrentEditor and readOnly', () => {
                const { result } = renderHook(() => useMediaItemHandling({ ...baseConfig, readOnly: true }))
            ...
            it("should be called when user is not current editor, but it isn't readonly either", () => {
                const { result } = renderHook(() => useMediaItemHandling({ ...baseConfig, userId: 'foo' }))
