...
const renderUserRoleState = (
  currentEditorId: TeamMemberId | undefined,
  teamMemberId: TeamMemberId
) => {
...
const TeamMembersListItem: React.FC<Props> = (props) => {
  const className = `presence__team-member presence__team-member${
    props.teamMember.connectionState === ConnectionState.CONNECTED
      ? '--connected'
      : '--disconnected'
  }`
  ...
  return (
    <li className={className}>
      <span tabIndex={0} aria-label={label}>
        {renderUserRoleState(props.currentEditor, props.teamMemberId)}{' '}
        {memberName}
      </span>
...
const mapStateToProps = (state: AppState, ownProps: OwnProps) => ({
  teamMember: selectors.presence.selectTeamMemberById(
    ownProps.teamMemberId,
    state
  ),
