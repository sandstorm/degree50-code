...
const PresenceSlice = createSlice({
  ...
  reducers: {
    ...
    setError: (
      state: PresenceState,
      action: PayloadAction<PresenceState['error']>
    ): PresenceState => ({
      ...state,
...
const selectTeamMemberById = (id: TeamMemberId, state: AppState) =>
  state.presence.teamMembersById[id]
const selectTeamMembersById = (state: AppState) =>
  state.presence.teamMembersById
const selectIsConnecting = (state: AppState) => state.presence.isConnecting

const selectOnlineTeamMemberIds = createSelector(
  [selectTeamMemberIds, selectTeamMembersById],
  (allIds, byId) =>
    allIds.filter(
      (memberId) => byId[memberId].connectionState === ConnectionState.CONNECTED
    )
)
