...
const eventStreamMessageAction = createAction(
  'Presence/Saga/eventStreamMessage'
)
...
const transformSubscriptionsToTeamMembers = (
...
const isSubscriptionResponse = (
  response: any
): response is { data: { subscriptions: Array<Subscription> } } => {
  return Array.isArray(response?.data?.subscriptions)
}

function* fetchSubscriptions() {
  const liveSyncConfig = selectors.liveSyncConfig.selectLiveSyncConfig(
    yield select()
  )
  const response: unknown = yield Axios.get(
    liveSyncConfig.subscriptionsEndpoint,
    {
      headers: {
  ...
  const subscriptions: Array<Subscription> = isSubscriptionResponse(response)
  ...
  const teamMembersFromState = selectors.presence.selectTeamMembersById(
    yield select()
  )

  // set all members as DISCONNECTED - only members with active subscription will be set as CONNECTED again
  const teamMembersFromStateDisconnected: Record<string, TeamMember> =
    selectors.presence.selectTeamMemberIds(yield select()).reduce(
      (acc, teamMemberId) => ({
        ...acc,
        [teamMemberId]: {
  ...
  const teamMembersInSubscription =
    transformSubscriptionsToTeamMembers(subscriptions)
...
function* promoteUserToCurrentEditor(
  action: ReturnType<typeof promoteUserToCurrentEditorAction>
) {
  const userId = selectors.config.selectUserId(yield select())
  const currentEditorId = selectors.currentEditor.selectCurrentEditorId(
    yield select()
  )
  ...
  if (userId !== currentEditorId) {
    try {
      Axios.post(
        selectors.config.selectConfig(yield select()).apiEndpoints
          .updateCurrentEditor,
        {
