  ...
  const filteredByFachbereich =
    (fachbereichFilters.length > 0
      ? videoFavorites.filter((videoFavorite) =>
          videoFavorite.video.fachbereiche.some((fachbereich) =>
            fachbereichFilters.includes(fachbereich.id)
          )
        )
      : videoFavorites) ?? []
  ...
  const filteredByCourse =
    courseFilters.length > 0
      ? filteredByFachbereich.filter((videoFavorite) =>
          videoFavorite.video.courses.some((course) =>
            courseFilters.includes(course.id)
          )
        )
