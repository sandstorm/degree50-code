...
export const selectUserCanEditSolution = createSelector(
  [
    ...
    dataSelectors.solutions.selectIsCurrentSolution,
  ],
...
const isAtCursor = (
  entity: { start: string; end: string },
  currentPlayPosition: number
) =>
  timeToSecond(entity.start) <= currentPlayPosition &&
  timeToSecond(entity.end) >= currentPlayPosition

const selectCurrentAnnotationIdsAtCursor = createSelector(
  [
    dataSelectors.selectCurrentAnnotationsByStartTime,
    videoEditorSelectors.player.selectSyncPlayPosition,
  ],
...
const selectCurrentVideoCodeIdsAtCursor = createSelector(
  [
    dataSelectors.selectCurrentVideoCodesByStartTime,
    videoEditorSelectors.player.selectSyncPlayPosition,
  ],
...
const selectCurrentCutIdsAtCursor = createSelector(
  [
    dataSelectors.selectCurrentCutListByStartTime,
    videoEditorSelectors.player.selectSyncPlayPosition,
  ],
...
const selectActiveVideoCodesFromPreviousSolutions = createSelector(
...
const selectActiveCutListFromPreviousSolutions = createSelector(
...
const selectAllAnnotations = createSelector(
  [
    selectActiveAnnotationsFromPreviousSolutions,
    dataSelectors.selectCurrentAnnotations,
  ],
...
const selectAllVideoCodes = createSelector(
  [
    selectActiveVideoCodesFromPreviousSolutions,
    dataSelectors.selectCurrentVideoCodes,
  ],
  (previousVideoCodes, currentVideoCodes) => [
    ...previousVideoCodes,
    ...currentVideoCodes,
  ]
)
...
const selectAllAnnotationsByStartTime = createSelector(
  [selectAllAnnotations],
  sortByStartTime
)
const selectAllVideoCodesByStartTime = createSelector(
  [selectAllVideoCodes],
  sortByStartTime
)
const selectAllCutsByStartTime = createSelector(
  [selectAllCuts],
  sortByStartTime
)

const selectAllAnnotationIdsByStartTime = createSelector(
  [selectAllAnnotationsByStartTime],
  (annotations) => annotations.map((a) => a.id)
)

const selectAllVideoCodeIdsByStartTime = createSelector(
  [selectAllVideoCodesByStartTime],
  (videoCodes) => videoCodes.map((vc) => vc.id)
)
...
const selectAllActiveVideoCodeIdsAtCursor = createSelector(
  ...
  (videoCodes, currentPlayPosition) => {
    return videoCodes
      .filter((vc) => isAtCursor(vc, currentPlayPosition))
      .map((vc) => vc.id)
  }
)
...
const selectAllActiveCutIdsAtCursor = createSelector(
  ...
  (cuts, currentPlayPosition) => {
    return cuts
      .filter((cut) => isAtCursor(cut, currentPlayPosition))
      .map((cut) => cut.id)
  }
)
