...
const MaterialSolutionSlice = createSlice({
  ...
  reducers: {
    setSelectedSolutionId: (
      state,
      action: PayloadAction<SolutionId | undefined>
    ) => ({
      ...state,
  ...
  extraReducers: (builder) =>
    builder
      ...
      .addCase(filterSlice.actions.setPreviousSolution, (state, action) => {
        const selectedSolutionIdHasBeenToggled =
          action.payload.id === state.selectedSolutionId
        const comparedSolutionIdHasBeenToggled =
          action.payload.id === state.comparedSolutionId
...
const selectSelectedSolutionId = (state: AppState) =>
  state.data.materialSolution.selectedSolutionId
const selectComparedSolutionId = (state: AppState) =>
  state.data.materialSolution.comparedSolutionId
const selectIsReadonly = (state: AppState) =>
  state.data.materialSolution.isReadonly
const selectShouldCompare = (state: AppState) =>
  state.data.materialSolution.shouldCompare
...
const selectCanCompareNextSolution = createSelector(
  ...
  (solutionIds, comparedSolutionId) =>
    Boolean(
      comparedSolutionId &&
        isNotLastElementInArray(solutionIds, comparedSolutionId)
    )
)
...
const selectCanComparePreviousSolution = createSelector(
  ...
  (solutionIds, comparedSolutionId) =>
    Boolean(
      comparedSolutionId &&
        isNotFirstElementInArray(solutionIds, comparedSolutionId)
    )
)
