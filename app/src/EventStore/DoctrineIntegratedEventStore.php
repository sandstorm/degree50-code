...
class DoctrineIntegratedEventStore implements EventSubscriber
{
    ...
    public function onFlush(OnFlushEventArgs $args)
    {
        ...
        if ($this->ensureEventsArePublishedForEveryObjectModification($args->getObjectManager()->getUnitOfWork()) === false) {
            return;
        }

        if ($args->getObjectManager()->getConnection()->getNestTransactionsWithSavepoints() === true) {
            $args->getObjectManager()->getConnection()->setNestTransactionsWithSavepoints(false);
    ...
    {
        if ($args->getObjectManager()->getConnection()->isRollbackOnly()) {
        ...
        if ($this->reenableSavepointsAfterTransactionIsComplete) {
            $args->getObjectManager()->getConnection()->setNestTransactionsWithSavepoints(true);
        }
